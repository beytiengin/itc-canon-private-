...
**Son güncelleme:** 15 Mayıs 2026 (Karar 21, Yıldız–Travma Ayrılması)
...
**Tetikleyici:** App'in temel yapısı incelendiğinde Yıldız Oyuncu Matrisi'nin yalnızca psikolojik boyutunun travma erişim eşiği olarak kullanıldığı, matrisin 6 boyutlu pedagojik amacının sistemde karşılığı olmadığı tespit edildi. Detay arşiv: `kararlar/2026-05-15-yildiz-travma-ayrismasi/`.
...
- §3.17 Yöntem Tarihsel Çizgisi: Filiz klinik altyapı listesinden "Yıldız Profil Eşleşme" çıkarıldı (tarihsel kayıt notu eklendi)
- v1.9 değişiklik bloğu + sürüm geçmişi satırı

**Etik Çerçeve (`.claude/rules/etik-cerceve.md`):**

- §3 "Yıldız Profil Eşleşme Matrisi" maddesi kaldırıldı (tarihsel rehber notu olarak yerinde tutuldu, numaralama korundu)
- §2 Travma Matrisi: uyarı sistemi yapısı eklendi (gate değil)
...
- `app/app/lib/travma.js` `sahneErisimi()` fonksiyonu `sahneUyarisi()` ve `topraklanmaGerekli(yogunluk)` olarak ikiye ayrılır
- Karakter sayfalarında (willy/macbeth/biff) `psikolojikPuan` props'u kaldırılır; tüm `yildiz` objesi taşınır
- `app/app/lib/kalibrasyon.js`: yeni `yildizYaklasimi(yildiz, karakter)` fonksiyonu (VAK'ın `vakDili` örüntüsünde)
- ZihinselAntrenman / SeninCerceven: gate mantığı kaldırılır
- UX metinleri (kalibrasyon/yildiz sayfaları): "psikolojik kapasiten travma erişimini belirler" çerçevelemesi kaldırılır

Detay: `kararlar/2026-05-15-yildiz-travma-ayrismasi/04-app-degisiklik-listesi.md`.
...
## 12 Mayıs 2026 — Filiz Kaya Ataklı'nın Eş Kurucu ve Eş Eğitmen Statüsü Kanon (Karar 16)

**Tip:** Kanon Değişikliği (Tip 1)
**Etkilenen katmanlar:** Tüm ekosistem (Method Book + Workbook + App + Workshop + Brief + Decision Log + tüm dış iletişim)
**Spine sürümü:** v1.6 → v1.7 (kanon değişikliği)
**Karar veren:** Beyti
**⚠ DURUM: Filiz yazılı onayı bekleniyor — uygulamaya geçer ama Filiz onay turunda kontrol edilecek; onay tarihi geldiğinde bu flag kaldırılır.**

> **Karar numaralandırma notu:** Bu karar Workshop sohbetinden "Karar 12" olarak gelmişti; ana Decision Log'da **Karar 16** olarak yedirildi (12 Mayıs paketinde Karar 15 = HMDK Brief tarih düzeltmesi olarak tahsis edilmişti, bkz. `kararlar/2026-05-12-workshop-geri-besleme-paketi/00-README.md`). Aşağıdaki metinde "Karar 11 (HMDK Stuttgart Bağlam)" atfı Workshop sohbeti karar sırasındandır ve ana repodaki Karar 11'den (Transmisyon Mimarisi: BE × FKA Eş Eğitmenlik) farklıdır. Arşivdeki orijinal dosya `kararlar/2026-05-12-filiz-es-kurucu-statusu/decision-log-eklentisi-karar-16.md` olarak korunmuştur.

**Tetikleyici:** Workshop projesi — Berlin Chubbuck Studio sayfasının (`https://www.chubbuckstudioberlin.com/characterdesign`) incelenmesinde Filiz'in saha pratiğindeki konumunun ("co-creator", "co-instructor") dokümantasyondaki konumundan ("klinik gözetim", "eş sahip") farklı olduğu tespit edildi.

### Karar

Filiz Kaya Ataklı, ITC ekosisteminin **eş kurucusu ve eş eğitmenidir**. Bu statü tüm dokümantasyonda, iç ve dış iletişimde tutarlı olarak yansıtılır. Dış iletişimde rolün açıklayıcı uzantısı "Klinik Psikolog" parantez içinde, statünün önünde ya da ardında, bağlama göre konulur.

### Mevcut Tutarsızlık Haritası

Karardan önce dokümantasyonda Filiz'in konumu tutarsızdı:

| Belge | Eski formülasyon |
|---|---|
| Brief v2 | "Klinik & Akademik Eş Sahip (Klinik Psikolog)" |
| Ekosistem Spine v1.5 | "Beyti Engin × Filiz Kaya Ataklı" (eşit görünür ama her zaman ikinci sırada) |
| Decision Log | "Klinik & Akademik Eş Sahip" |
| Method Book Project Instructions | "klinik ve pedagojik gözetim" |
| ITC Workshop Project Instructions v1.1 | "klinik ve pedagojik gözetim" |
| Karakter Tasarım 2021 kitabı (kapakta) | "Filiz Kaya & Beyti Engin" (Filiz önce — eşit yaratıcı tutumu en net burada) |
| Chubbuck Studio Berlin sitesi | "Filiz Kaya Ataklı and Beyti Engin, creators of the Character Design Technique" (eşit kurucu — saha pratiği) |
| Method Book yapısı (Spine §2) | "çift sesli — Beyti Engin × Filiz Kaya Ataklı" |
| Brief v2 Milestone 1.4 | "Filiz Kaya Ataklı'nın adı ve fotoğrafı 'Klinik Eş Sahip & Akademik Ortak' olarak ana sayfada" |

Saha pratiği ile dokümantasyon arasındaki bu uçurum bu kararla kapatılır.

### Yeni Kanon Formülasyonu

**Tam unvan (resmî bağlamlar, yayın, akademik, kurumsal):**
> Filiz Kaya Ataklı (Klinik Psikolog) — Eş Kurucu ve Eş Eğitmen
> Beyti Engin — Eş Kurucu ve Eş Eğitmen

**İngilizce (uluslararası bağlamlar):**
> Filiz Kaya Ataklı (Clinical Psychologist) — Co-Founder & Co-Instructor
> Beyti Engin — Co-Founder & Co-Instructor

**Almanca (HMDK Stuttgart, DACH bölgesi):**
> Filiz Kaya Ataklı (Klinische Psychologin) — Mitbegründerin & Co-Trainerin
> Beyti Engin — Mitbegründer & Co-Trainer

**Kısa yan yana referans (iç dokümantasyon, başlık, sürüm bilgisi):**
> Beyti Engin & Filiz Kaya Ataklı

Sıralama: TR ve resmî belgelerde tarihsel olarak BE önce gelebilir (yöntem yaratıcısı ve birincil pratisyen olarak); ancak EN/DE bağlamlarda **FKA önce de gelebilir** (Karakter Tasarım 2021 kapak örneği ve Berlin sitesi örneğindeki gibi). Sıralama bağlama göre değişebilir, ama **her ikisi de "Eş Kurucu" olarak işaretlenir** — bu sabittir.

İki nokta ("·") veya çarpı ("×") yerine **& işareti** kullanılır — ortaklığı vurgular. ("Beyti Engin × Filiz Kaya Ataklı" eski formattı; yeni "Beyti Engin & Filiz Kaya Ataklı".)

> **Yedirme notu (ana repo):** "×" → "&" değişikliği yazar/kurucu bağlamında uygulanır (Spine üst meta, footer, Brief önsöz/üst meta, Decision Log üst meta, README). Pedagojik teknik formülasyon olan §3.13 başlığı "BE × FKA Eş Eğitmenlik" korunmuştur (eş eğitmenlik dağılımının teknik formülü).

### Etik Gerekçe

Bu karar üç katmanda gerekli:

**1. Saha gerçekliğine dokümantasyonu hizalama.** Filiz pratikte 2005'ten bu yana yöntemin yaratımına eşit katılım göstermiş. Saha pratiği Berlin'de, akademik referanslarda, Karakter Tasarım kitabında bunu zaten yansıtıyor. İç dokümantasyon geriden geliyordu.

**2. Klinik altyapının görünür kalması.** "Klinik gözetim" ifadesi Filiz'in rolünü destek statüsüne indiriyordu — oysa ITC'nin etik altyapısı (Travma Matrisi 8×4, Yıldız Eşleşme, Topraklanma Protokolü, Substitution Yasağı) **Filiz'in klinik çerçevesi olmadan yöntem değil sadece teknik kalır**. Eş kurucu statüsü bu omurganın görünürlüğüdür.

**3. Tek-kişi-bağımlılığı riskinin azaltılması (Brief v2 §2.4).** Brief v2 risk profilinde "Beyti tek-kişi-bağımlılığı: Yüksek/Kritik" işaretliydi. Filiz'in eş kurucu statüsünün kanonlaşması bu riski azaltır — ekosistem iki imzayla taşınır, tek isimle değil.

### Uygulama — Tüm Katmanlarda

**Spine v1.6 → v1.7:**
- Belge başlığındaki "Inside The Character · Beyti Engin × Filiz Kaya Ataklı" formülasyonu "Inside The Character · Beyti Engin & Filiz Kaya Ataklı — Eş Kurucular ve Eş Eğitmenler" olarak güncellenir
- §1 ekosistem haritası açıklamasına bir not eklenir: "ITC, Beyti Engin ve Filiz Kaya Ataklı tarafından 2005'ten bu yana birlikte geliştirilen bir yöntemdir. Her iki kurucu da metodoljinin eşit yaratıcı ortağıdır; Filiz'in klinik psikoloji çerçevesi yöntemin etik omurgasının ayrılmaz parçasıdır."
- Sürüm Geçmişine v1.7 satırı

**Method Book:**
- Project Instructions §1 yazar tanımı güncellenir
- Kitabın kapağında ve iç sayfasında "by Beyti Engin & Filiz Kaya Ataklı" — iki yazar eşit görünür
- Telif sayfasında her ikisi de eşit yazar olarak listelenir
- Bölüm D (Etik Altyapı) açılışında Filiz'in klinik çerçevesinin yöntemin yaratımındaki rolü açıkça anılır

**Workbook 2. baskı:**
- Kapak içinde mevcut yazar bilgisi gözden geçirilir; eğer "FKA klinik gözetim" formülasyonu kullanılıyorsa "Eş Kurucu" olarak güncellenir
- Errata listesine eklenir

**App (`itc-app-six.vercel.app`):**
- "About" / "Hakkında" sayfası güncellenir
- Brief v2 Faz 1 Milestone 1.4'teki "Filiz Kaya Ataklı'nın adı ve fotoğrafı 'Klinik Eş Sahip & Akademik Ortak' olarak ana sayfada" hedefi artık **"Eş Kurucu ve Eş Eğitmen"** ifadesiyle yapılır
- Onboarding akışında ITC anlatımında iki kurucu eşit görünür

**Workshop Spine v1.1 → v1.2 (yapım sürmekte):**
- Tüm format kartlarında ve master outline'larda eş eğitmenlik tablosu "Eş Kurucular ve Eş Eğitmenler" başlığı altında yazılır
- A2-intensive HMDK Master Outline v2.1 ve sonraki sürümleri bu kanona göre

**Workshop Project Instructions v1.1 → v1.2:**
- Proje tanımında "Yöntem Eş Yazarı: Filiz Engin (klinik ve pedagojik gözetim)" formülasyonu **"Yöntem Eş Kurucusu ve Eş Eğitmeni: Filiz Kaya Ataklı (Klinik Psikolog)"** olarak güncellenir
- Soyadı tutarlılığı: bazı yerlerde "Filiz Engin" geçiyor, bu yanlış — doğrusu **"Filiz Kaya Ataklı"** (her yerde tutarlı kullanılır)

**Brief v2 → v2.2:**
- Önsöz yazar bilgisi güncellenir
- Risk profili tablosunda "Beyti tek-kişi-bağımlılığı" satırı azaltma sütununa "Filiz Kaya Ataklı eş kurucu statüsünün kanonlaşması (Karar 16)" eklenir
- §2'de "Method Kitabı çift sesli — Beyti Engin × Filiz Kaya Ataklı" formülasyonu güncellenir
- §10.4 akademik hedeflerde "FKA klinik onayı" formülasyonu "eş yazar imzası" düzeyine yükseltilir

**Decision Log (bu doküman):**
- Bundan sonraki tüm kararlarda "Karar veren" satırı sadece Beyti ise, klinik veya pedagojik içerik içeren kararlar için **"FKA onayı bekleniyor / alındı"** notu eklenir. Etik altyapı ve travma içerikli kararlar Filiz'in onayı olmadan kanonlaşmaz.

**Dış iletişim (web, sosyal medya, basın, akademik):**
- `beytienginstudio.com` ve diğer kanallarda ITC tanıtımları güncellenir
- LinkedIn ve diğer profesyonel profillerde iki kurucu görünür
- Akademik makale yazımında (Brief v2 Faz 2 Milestone 2.4) eş yazar olarak Filiz birinci veya eşit pozisyonda listelenir
- Konferans sunumlarında "by Beyti Engin & Filiz Kaya Ataklı" formülasyonu

### Berlin Chubbuck Studio'ya Etki

Berlin sitesindeki mevcut "Filiz Kaya Ataklı and Beyti Engin, creators of the Character Design Technique" formülasyonu Berlin'in iletişiminde **doğru tutumdur** — sadece "Character Design" yerine "Inside The Character (ITC)" markası kullanılır (gelecek karar — marka kanonu). Bu satır şöyle güncellenir:

> Filiz Kaya Ataklı (Clinical Psychologist) & Beyti Engin, co-founders and co-instructors of Inside The Character (ITC), formerly known as Character Design.

### HMDK Stuttgart'a Etki

Pia Podgornik ve Katherina [soyad?] ile koordinasyonda Filiz'in fiziksel mevcudiyeti (5 gün Stuttgart'ta — Workshop sohbeti Karar 11 Madde 6) yalnızca klinik destek değil **eş eğitmen** olarak konumlandırılır. Atölye kontratında ücretlendirme buna göre yapılır — saatlik ücret iki eş eğitmene ayrı ayrı uygulanır.

### Notlar

- Bu karar Workshop sohbetinin Karar 11'inden (HMDK bağlam) hemen sonra geldi çünkü Berlin Chubbuck Studio sayfası incelenirken **saha pratiği ile dokümantasyon arasındaki uçurum** belirgin oldu. Bu yapısal bir karardı — pazar gerçekliği zaten dokümantasyondan ileri konumdaydı.
- Karar metni Filiz tarafından okunmalı ve onaylanmalıdır. Yazılı onay alınana kadar bu karar **uygulamaya geçer ama Filiz onayı bekleniyor** notuyla işaretli kalır. Onay geldiğinde bu not silinir, Decision Log'a Filiz'in tarihli onayı eklenir.
- Soyad düzeltmesi kritik: "Filiz Engin" gibi yanlış kullanımlar tüm geçmiş dokümantasyonda taranıp düzeltilmelidir. Doğru kullanım: **Filiz Kaya Ataklı** (her zaman üç kelime, kısaltma yapılmaz). Ana repodaki bilinen yanlış kullanım (Karar 11 metni içinde) bu yedirme sırasında düzeltildi.
- Karakter Tasarım 2021 kitabı kapağındaki "Filiz Kaya & Beyti Engin" sıralaması bu kararın tarihsel kökeni — 2021'de zaten Filiz birinci sırada konulmuştu. Bu karar yalnızca dokümantasyonu o gerçekliğe hizalar.

### Bağlı Kararlar

- Workshop sohbeti Karar 11 (HMDK Stuttgart Bağlam): FKA'nın 5 gün fiziksel mevcudiyeti eş eğitmen rolüne göre yeniden değerlendirilir.
- Gelecek karar: Berlin Chubbuck Studio yapısı (eş eğitmen olarak ikiniz birden, Auditor + Participant + Advance kademeleriyle).
- Gelecek karar: Ekosistem marka kanonu — "Inside The Character (ITC)" tek marka, "Character Design" emekliye ayrıldı (Berlin'de tarihsel kullanım dahil).
...
Filiz Kaya Ataklı'nın (Klinik & Akademik Eş Sahip — Karar 16 ile Eş Kurucu ve Eş Eğitmen olarak yeniden tanımlandı) ekosistemdeki konumu — Karar 11 ile birlikte — net hâle gelir:
