# ITC EKOSİSTEM — SPINE

**Dört projenin paylaştığı omurga**
**v1.5 · 12 Mayıs 2026 · Beyti Engin × Claude**

---

## v1.3'te neler değişti?

v1.2'ye göre iki büyük yapısal güncelleme yapıldı:

1. **Workshop dördüncü katman olarak ekosisteme eklendi.** Ekosistem artık dört katmanlıdır: Method Book / Workbook / App / Workshop. Workshop, Method Book'un D bölümünden türeyen operasyonel mimari katmanıdır — eğitmenin elinde fiilen kullandığı program kiti. §1, §2, §4 bu genişlemeye göre güncellendi.

2. **Method Book ana yapı oturumunda 8 stratejik karar alındı.** 8 Mayıs 2026 tarihli oturum (Decision Log'da kayıtlı) §3'e üç yeni alt-bölüm ekledi (§3.9 Modül I'in Üç Testi · §3.10 Doğrular Üç Kaynak Yapısı · §3.11 Dijital Araç Sınırı İlkesi), §2 Method Book yapısını güncelledi (C-Ek bölümü eklendi), §5 açık kararlarını azalttı.

v1.2'nin geri kalan içeriği aynen korundu. §6 ve §7 değişmedi.

---

## 1 · Ekosistemin Haritası

ITC tek bir ürün değil — dört katmanı olan bir sistem:

| Katman | Form | Hedef kitle | İşlev |
|---|---|---|---|
| **Method Kitabı** | Basılı kuramsal+pratik kitap (~320 sf) | Eğitmenler, ileri oyuncular, akademi | Foundation. Yöntemin kavramsal otoritesi. |
| **Hamlet Workbook** | Basılı uygulamalı çalışma kitabı (314 sf) | Tek başına çalışan oyuncu, prova öncesi | Yöntemin tek karaktere derinleşmiş hâli. |
| **ITC App** | Web uygulaması, mobil uyumlu | Günlük antrenman yapan oyuncu | Dijital günlük pratik. Kalibrasyon + kısa antrenman. |
| **Workshop** | Program şablonları + eğitmen kiti | Eğitmen, kurum (akademi, atölye) | Yöntemin operasyonel mimarisi. Kurumsal ölçeklenme. |

### Konumlandırma — "Ticari ama akademik duran" + Kurumsal Omurga

Dört katmanlı yapı, ekosistemin doğal konumlandırmasını içeriyor:

- **Method Kitabı** — akademik prestij taşıyıcısı (akademi, eğitmenler)
- **Hamlet Workbook** — profesyonel oyuncuya derin pratik (basılı + dijital)
- **ITC App** — günlük pratik yapan oyuncuya bireysel araç
- **Workshop** — eğitmen ve kurum için operasyonel program kiti (kurumsal ölçeklenme)

Bu yapı "ticari ama akademik duran" sentezi sağlıyor: Method Book yöntemin akademik temelini kurar, App+Workbook ticari geliri sağlar, Workshop kurumsal pilotları (HMDK Stuttgart, Royal Central, Stella Adler, USC, İstanbul Bilgi vb.) yürütür. Dördü birbirini destekler.

---

## 2 · Dört Projenin Güncel Durumu

### Method Kitabı — Inside The Character

**Durum:** Master Outline v3 hazırlanmakta (8 Mayıs 2026 oturum sonrası). Manuscript yazımına henüz başlanmadı.

**Yapı:**
- A. Giriş
- B. Birinci Bölüm: Felsefe (Kabuller + Manifesto + Kavramsal Çerçeve + Bilimsel Temeller)
- C. İkinci Bölüm: Üç Modüllü Sistem (Modül I, II, III)
- **C-Ek. Performans Blokajları ve Çözümleri** (YENİ — 8 Mayıs 2026, Modül III'ün dışında ek bölüm)
- D. Üçüncü Bölüm: Etik Altyapı ve Eğitmen Rehberliği
- E. Dördüncü Bölüm: Uygulama Vakaları
- F. Ekler (Sözlük, Karaktere Hazırlık Soruları, Öğrenci Defteri, formlar)

**Yazarlar:** Beyti Engin × Filiz Kaya Ataklı (çift sesli)
**Hedef:** ~320 sayfa
**Üretim sırası:** Bölüm III prototip → I → II → IV → VI → V → Açılış+Kapanış sona

### Hamlet Workbook — Inside The Hamlet

**Durum:** Sürüm 1.0 yayımlandı. 2. baskı hazırlığında.
**Yapı:** 5 kısım · 13 yolculuk bölümü · 5 çapa · 3 bedensel yoklama · 314 sayfa
**Eyüboğlu çevirisi:** Türkiye İş Bankası Kültür Yayınları izniyle

**Sonraki adım — 2. baskı errata:**
- s.310 "11 yolculuk" → 13 düzeltilir
- TOC tutarsızlığı düzeltilir
- Method Book'a geri-referanslar eklenir (sayfa numarası eşleşmeleri)
- Klinik güvenlik notları eklenir (Eşik'e helpline notu, dört-beş soruluk öz-tarama, Yoklama'lara "iyi değilsen" dalı, "terapi değildir" disclaimer'ı)
- Bölüm IX çapa cümlesi için ek koruma katmanı

### ITC App — Actor's Gym

**Durum:** Canlı (erken beta). `itc-app-six.vercel.app`

**Modül I — Kendini Tanı:** ✅ CANLI · 3 test (VAK 27, MBTI 12, Yıldız 37) · ~15 dk
**Modül II — Karakterini İnşa Et:** ✅ CANLI · 4 karakter · 30 antrenman · 25-30 dk/antrenman

**Modül II Hamlet Refactor (v1.2 yeni karar):**
- Yeni sıralama: Doğrular (Workbook s.50-57) → Timeline (s.58-85) → Yazarın Çerçevesi (s.86-105) → Senin Çerçeven (s.106-125)
- Pilot Hamlet'te → sonra Macbeth, Willy, Biff
- Eski 9 antrenman retire edilmez, yeni yapıya entegre

**Modül III — Sahnele (Yolculuk):** ⏳ YAKINDA
- Pilot: Workbook 13 yolculuk metni (s.126-289), ~110 dk AI Dış Ses
- Topraklanma overlay: Workbook s.288-289
- TTS altyapısı: ElevenLabs Multilingual v2 (test aşamasında)

### Workshop — Operasyonel Mimari

**Durum:** Spine v0 hazırlanıyor (ayrı Workshop sohbeti yürütülecek).

**Yapı:**
- A1 — Tanıtım (3-4 saat)
- A2 — Tam Modül atölyesi (amiral gemisi)
- B1 — Kendi Karakterin (uygulama atölyesi)
- C1, C2, C3 — Niş atölyeler (Metin Çözümleme, Karakter Sayfası Mimarisi, Akış & Performans)
- D1 — Etik & Klinik Güvenlik (FKA imzalı)
- E1 — Train-the-Trainer (eğitmen sertifikasyonu — yapı henüz açık)

**Hammadde:** 8-haftalık Detaylı Eğitim Programı Rebuild Final, Pakademi 24 hafta müfredatı, HMDK Stuttgart Participants Handbook, Berlin Sunum 2023, Macbeth ders transkripti, Akış Modülü FINAL.

**Eksik:** 4-haftalık ve 12-haftalık program şablonları (8-haftalıktan türetilecek).

---

## 3 · Sabit ITC İlkeleri (Dört projede de aynı)

Aşağıdaki ilkeler ITC'nin değişmez omurgasıdır. Dört projede de aynen geçerlidir; bir projede aksine bir şey yazıyorsa hata orada.

### 3.1 · Substitution kullanılmaz

ITC oyuncunun kendi biyografik travmalarına dönmesini istemez. Karakterin verisi karakterin kendisinden çıkarılır; oyuncunun yarasından beslenmez. Kanonik formülasyon:

> Karakter senin kişisel travmalarınla inşa edilmez. Karakterin kendi verisi vardır. — Etik motto

> Sahip değil — misafir. — Workbook s.18, Kabul 4'ün uygulamalı motto formu

ITC ile ilgili tüm sorular ve materyaller oyuncuyu karakterin dünyasına davet eder — kendi hayatına geri değil.

### 3.2 · Travma matrisi: 8 kategori × 4 yoğunluk

Dört projede de aynı sınıflandırma kullanılır.

**Kategoriler:** Şiddet · Kayıp · İhanet · Cinsel · Zihinsel Kırılma · Çocukluk · Ahlaki Yara · Varoluşsal
**Yoğunluk:** 0 Yok · 1 Atıf · 2 Tanıklık · 3 İcra

"0 Yok" seviyesi etik altyapının önemli bir parçası — bazı antrenmanların travmatik içermediğini açıkça işaretlemek için.

### 3.3 · Yıldız Profil Eşleşme Matrisi

Psikolojik puan eşikleri:
- ≥5: Tüm seviyeler açık
- 3–5: Uyarılı
- <3: "Hazır olduğunda" — kilit değil bekleyiş

### 3.4 · Topraklanma Protokolü (6 adım)

Method Book'taki jenerik form. Workbook s.288-289'da Hamlet'e özel adaptasyon var. App'te Modül III'e otomatik overlay olarak gelecek (ardışık 2 seviye 2 antrenman sonrası).

1. Nefes (4-4-6, üç tekrar)
2. Bedene dönüş (eller, yüz)
3. İsim (üç kez yüksek sesle)
4. Tarih (gün-ay-yıl)
5. Mekân (5 farklı renk/nesne)
6. Kapanış cümlesi: "Karakter orada — ben buradayım."

### 3.5 · Karakter Sayfası Mimarisi

Karakter Kimliği → Değiştirilemez Doğrular → Timeline → Yazarın Çerçevesi → Senin Çerçeven → Zihinsel Antrenman

**Not:** Modül II Hamlet refactor sonrası (v1.2) bu mimari Hamlet için "Karakter Kimliği → Doğrular → Oyun Öncesi Yaşam → Timeline → Yazarın Çerçevesi → Senin Çerçeven → Modül III CTA" şeklinde genişledi. Diğer üç karakter (Macbeth, Willy, Biff) hâlâ eski mimaride; refactor onlara sonra uygulanacak (Faz 3 Milestone 3.2).

### 3.6 · Üç Giriş Kapısı

Bilişsel · Bedensel · Duygusal. VAK'tan farkı: nasıl öğrendiğin değil, karaktere nereden girdiğin.

Karaktere Hazırlık Soruları'nın (Method Book Bölüm F) yapısal omurgası bu üç kapıdır (bkz. §3.10 ile birlikte ve Karar 8).

### 3.7 · Altı Kabul

1. Karakter analiz edilmelidir.
2. Hiçbir analiz analiz edenden bağımsız değildir.
3. Oyuncu kendini de analiz etmelidir.
4. Karakter oyuncudan bağımsız yaratılmalıdır — "Sahip değil, misafir."
5. Tek yöntem yoktur — kişiye özel giriş kapıları vardır.
6. Zihin-beden bütünlüğü.

### 3.8 · Kuramsal Zemin

Damasio (somatik belirteçler — Kabul 6) · Goleman (duygusal zekâ) · Gardner (çoklu zekâ — Kabul 5) · Levine (somatik travma — Kabul 4) · Csikszentmihalyi (akış — Modül III'ün Akış katmanı).

### 3.9 · Modül I'in Üç Testi (YENİ — v1.3, Karar 1+5)

Modül I (Kalibrasyon) üç bağımsız test üzerine kurulur. Her test farklı bir boyutu ölçer ve App-only olarak uygulanır (bkz. §3.12):

**VAK — Öğrenme Stili Analizi**
- Soru sayısı: 27 · Süre: ~5 dakika
- Ölçtüğü: Bilgiyi içselleştirme kanalı — Görsel, İşitsel, Kinestetik
- Sonuç: Üç kanaldan baskın olanı belirlenir
- Pedagojik kullanımı: Zihinsel antrenmanların "derinleşme" adımı oyuncunun baskın kanalına göre şekillenir.

> **Önemli ayrım:** VAK ≠ Giriş Kapıları. VAK "nasıl öğrendiğini" söyler (algı kanalı). Karaktere "nereden girdiğini" (Bilişsel · Bedensel · Duygusal) Modül II'deki Giriş Kapıları belirler. İki üçlünün kesişimi 3×3 Matris'tir — bkz. §3.10.

**MBTI — Kişilik Tipi Analizi**
- Soru sayısı: 12 senaryo · Süre: ~5 dakika
- Ölçtüğü: Düşünme biçimi, karar verme yapısı, dünyaya bakış (Jungiyen 4 eksen: E/I, S/N, T/F, J/P)
- Sonuç: 16 kişilik tipinden biri
- Pedagojik kullanımı: Karakterin MBTI'sıyla oyuncununki karşılaştırılır; uzaklık anlam üretir.

**Yıldız Oyuncu — 6 Boyut Profili**
- Soru sayısı: 37 · Süre: ~7 dakika
- Ölçtüğü: 6 boyut — Teknik · Psikolojik · Mesleki · Yaratıcı · Entelektüel · İlişkisel
- Sonuç: Her boyutta puan, "psikolojik" boyutu travma erişim eşiğini belirler (§3.3)
- Pedagojik kullanımı: Oyuncunun "yıldız" konumuna mesafesi gösterir; periyodik tekrar gerekir.

### 3.10 · Çift Üçlü İlkesi ve 3×3 Matris (YENİ — v1.4, Karar 9)

ITC içinde iki ayrı kanonik üçlü bulunur. Bu iki üçlü zaman zaman birbirinin yerine geçecek biçimde kullanılmıştır; bu kullanım hatalıdır ve pedagojik hataya yol açar. İlke şudur: iki üçlü farklı sorulara cevap verir, farklı modüllerde operasyonel hale gelir, farklı uygulama biçimine sahiptir. Çarpımları (3×3) eğitmen ve oyuncu için somut bir başlama matrisi üretir.

**İki Üçlünün Karşılaştırması**

| Boyut | VAK Üçlüsü | Giriş Kapıları Üçlüsü |
|---|---|---|
| Üçlü | Görsel · İşitsel · Kinestetik | Bilişsel · Bedensel · Duygusal |
| Sorduğu soru | Nasıl öğrenirsin? | Karaktere nereden girersin? |
| Ölçtüğü | Bilgi işleme kanalı (algı–içselleştirme) | Karaktere erişim kanalı (analiz–temas) |
| Kanonik yeri | Modül I — Kendini Tanı (Kalibrasyon) | Modül II — Karakter Dramaturjisi |
| Uygulama | App-only test (27 soru · ~5 dk) | Soru havuzu — kâğıt-kalem (Karaktere Hazırlık Soruları) |
| Dijital Araç Sınırı (§3.12) | App-only — algoritmik puanlama | Method Book Bölüm F — birey kâğıtla çalışır |
| Dördüncü kanal | **Sezgisel** (gözlemsel okuma; algoritmaya girmez) | Yok — üçlü kapalıdır |
| Pedagojik kullanım | Zihinsel antrenmanların "derinleşme" adımı baskın kanala göre | "Bir gece / bir hafta" sınırı altında en hızlı doğru soruyu bulma |

**Yaygın Hatanın Anatomisi**

Karışımın klasik biçimi: *"VAK'a göre karaktere bedenden giriyor."*

- **Hata:** "VAK = Kinestetik" ile "Giriş Kapısı = Bedensel" aynı şey sanılmıştır. Aynı şey değildir.
- **Doğru okuma:** Görsel öğrenen bir oyuncu, karaktere bilişsel, bedensel veya duygusal kapıdan girebilir. Bunlar farklı katmanlardır ve birbirinden bağımsız işler.
- **Örnek:** Görsel-baskın bir oyuncu (VAK) → Macbeth'e bilişsel kapıdan girer (Giriş Kapısı) → derinleşme aşamasında karakteri "kafasında resmederek" çalışır (VAK'ın baskın kanalı bu noktada devreye girer). Kapı bilişsel olduğu için soruları analitik-yapısaldır; oyuncu görsel olduğu için derinleştirme sahnenin gözle kurulmasıdır.

**3×3 Matris**

İki üçlünün kesişimi 9 hücreli bir matris üretir. Her hücre, oyuncunun "açık kanalından açık kapıya" geçişinde önerilen başlangıç egzersizini taşır. Matris kullanımı: oyuncu VAK kalibrasyonundan (Modül I) baskın kanalını öğrenir; Karaktere Hazırlık Soruları (Modül II / Method Book Bölüm F) ile baskın kapısını bulur; matriste karşılık gelen hücreye bakar.

|                  | **Bilişsel Kapı**                          | **Bedensel Kapı**                              | **Duygusal Kapı**                                  |
|------------------|--------------------------------------------|------------------------------------------------|----------------------------------------------------|
| **Görsel**       | Sahne diyagramı · zihin haritası · timeline çizimi | Karakterin silueti · postür eskizi · ayna çalışması | Yüz/beden ifadesi okuma · karakter portresi etüdü |
| **İşitsel**      | Replik analizi · tonlama yapısı · monolog parçalama | Nefes-ses ritmi · vokal warm-up · prosodi çalışması | Ses kalitesi-duygu eşleştirme · iç ses dinleme   |
| **Kinestetik**   | Yapıyı bedenle deneyimleme · blocking provası | Hareket etüdü · somatik bedensel yoklama · yürüyüş | Karakterin bedensel duygusu · imgesel temas      |

**Önemli — etik kayıt:** "Kinestetik × Duygusal" hücresinde *karakterin* bedensel duygusu çalışılır, oyuncunun kendi anısı değil. Bu Substitution kullanılmaz ilkesinin (§3.1) matris üzerinde işaretidir.

**Sezgisel Dördüncü Kanal**

VAK üçlüsünün yanında **Sezgisel** kanal dördüncü olarak konumlanır. Bu kanal Karakter Tasarım 2021 kitabında ("görebildiğiniz · işitebildiğiniz · tadabildiğiniz · dokunabildiğiniz · koklayabildiğiniz" duyu açıklamasının üzerinde "bir düşünce akışı" olarak işaretli) ve Hamlet Workbook s.31–45 sezgisel temas bölümünde yer alır.

Sezgisel kanal algoritmik puanlamaya girmez; VAK testi 27 soruluk Görsel–İşitsel–Kinestetik puanlamasıyla sınırlıdır. Sezgisel, eğitmenin atölyede ya da oyuncunun günlüğünde gözlemsel olarak farkına vardığı bir katmandır. Bu, ITC'nin VAK testini sınır olarak işaretler — testin yeterince ölçemediği bir kanal vardır ve metot bunu reddetmez.

Sezgisel kanal matriste dördüncü bir satır açmaz — kendi başına bir okuma kanalıdır; matrise sezgisel okumanın notları kenardan eklenir.

Giriş Kapıları üçlüsü **kapalıdır**. Bilişsel · Bedensel · Duygusal'a "Sezgisel" dördüncü olarak eklenmez. Giriş Kapıları operasyonel olarak üç kapıyla çalışır ve bu üçlü Method Book Bölüm F'deki 9 adımlık Karaktere Hazırlık Soruları haritasının omurgasıdır.

**Karaktere Hazırlık Soruları ile Bağ**

Karaktere Hazırlık Soruları (Method Book Bölüm F · 2023 Berlin 9 adımlık akış) Üç Giriş Kapısı ile yapılandırılır. Her adımda kapılardan biri (ya da birden fazlası) baskındır; her kapı için ayrı soru seti bulunur.

- **Bilişsel kapı** — analitik / yapısal sorular (örn. "Karakterin değiştirilemez doğruları nelerdir?")
- **Bedensel kapı** — somatik / duyusal sorular (örn. "Karakter bir odaya girdiğinde bedeni nasıl hareket eder?")
- **Duygusal kapı** — ilişkisel / etkilenim soruları (örn. "Karakter babasını anlattığında bedeninde ne oluyor?")

Bu soru havuzu kâğıt-kalemle çalışıldığı için Method Book'ta yer alır — Dijital Araç Sınırı İlkesi §3.12 gereği. App'te bulunmaz; App'in karşılık gelen alanı "Senin Çerçeven" boşluk yansımalarıdır (Workbook s.106–125).

**Klinik Zemin (FKA)**

Giriş Kapısı seçiminin Duygusal'a yanlışlıkla zorlanması (örneğin "duygusal kanalı zayıfsan oradan başla, güçlendir" mantığı) oyuncuyu kendi duygu havuzuna geri çekme riski yaratır. ITC bu mantığı reddeder: oyuncu hangi kapıyı seçtiyse seçsin, karakterin verisinde kalır — kendi yarasında değil.

3×3 Matris bu açıdan bir güvence katmanıdır: matris hücreleri "açık kanaldan açık kapıya" geçişi somut egzersizle gösterir; oyuncuyu kendi geçmişine değil, karakterin verisine yönlendirir. Eğitmen bir kapıdan giriş "denenip" sonuç alınmadığında diğer kapıya geçer — bu test-deneme akışı Modül II/Aşama 2'nin operasyonel ritmidir.

Bu açıdan Çift Üçlü ayrımı sadece pedagojik değil, aynı zamanda etik bir kanondur. Substitution kullanılmaz ilkesi (§3.1) ile doğrudan bağlıdır.

### 3.11 · Doğrular Çift Üçlü ve 3×3 Matris (YENİ — v1.5, Karar 10 — Karar 3'ün genişletilmiş hâli)

> **Not:** Bu matris **§3.10'daki VAK × Giriş Kapıları matrisinden farklıdır** — karışmamalıdır. §3.10 oyuncu kalibrasyonu × karakter erişimini; §3.11 doğrunun kaynağı × ontolojik düzeyini ölçer.

Değiştirilemez Doğrular **iki ayrı eksende duran iki üçlü** ve onları birleştiren bir matrisle çalışılır. Tek üçlü, doğrunun çift karakterini gizler.

#### A. Kaynak Üçlüsü — *Doğru nereden geliyor?*

Oyuncuya **tartışma alanını** verir: neyle uğraşabilir, neyle uğraşamaz.

1. **Metin içi · doğrudan** — yazarın açıkça verdiği bilgi (replikler, sahne yönergeleri, açık karakter tanımları). Yoruma kapalı.
2. **Süreç içi · örtük** — metnin akışından, eylemler/alt-metinler/sahne bağlamları yoluyla çıkan. Sözle tam ifade edilmez ama metin zorunlu kılar.
3. **Yaratımsal · sahnelemeye bağlı** — yönetmen, dramaturg, oyuncunun ortak kararıyla kurulan. Bir sahneleme için karar verilince o sahneleme için değiştirilemez olur.

#### B. Katman Üçlüsü — *Doğru neye dair?*

Oyuncuya **çalışma yerini** verir: bedeniyle nereye yerleştirecek.

1. **Metinsel Doğrular** — replikte, sahne yönergesinde, açık karakter tanımında yaşar. Sözün doğrusudur.
2. **Dramaturjik Doğrular** — dünyanın yapısında: olay örgüsü, tarih, mekân, sosyal bağlam. Dünya kurulumunun doğrusudur.
3. **Zihinsel Sabitler** — karakterin iç inanç sisteminde: kararlar, ilkeler, gramerler. İç sesin doğrusudur.

**Not:** "Zihinsel Sabitler" terimi karışıklık yaratabilir: oyuncunun değil, karakterin zihninde sabit olan ilkeleri ifade ediyor.

#### C. Matris

İki eksen birlikte 3×3'lük matris oluşturur. Aynı doğru hem Kaynak hem Katman ekseninde okunur:

|                                  | **Metinsel** | **Dramaturjik** | **Zihinsel Sabit** |
|----------------------------------|--------------|-----------------|---------------------|
| **Metin içi · doğrudan**         | Hamlet prens; Wittenberg mektubu | Cenaze ve düğün arasında 2 ay | "Bana düşmez olaydı dünyayı düzeltmek" |
| **Süreç içi · örtük**            | Polonius'un kontrol dili | Saray Wittenberg'i geride bıraktı | Hamlet için intikam ahlaken muğlak |
| **Yaratımsal · sahnelemeye bağlı** | "Olmak ya da olmamak"ın yeri | Modern dönem mi, Rönesans mı? | Hamlet ateist mi, dindar mı? |

#### D. Pedagojik Sıralama

İki üçlü aynı anda öğretilmez:

- **Başlangıç** (Workbook, atölye D1): Kaynak Üçlüsü tek başına çalışılır
- **Orta-seviye** (Method Book §III/Aşama 1, atölye D2): Katman Üçlüsü tanıtılır, Kaynak ile ilişkisi açıklanır
- **İleri** (Method Book §III/Aşama 1 derinleşmesi, atölye C2): Matris bütünüyle uygulanır

#### E. Karakter Örnekleri

**Hamlet — "Bütün mesele hazır olmakta":** Kaynak ekseninde *metin içi · doğrudan* (V.2'de Horatio'ya replik). Katman ekseninde *Zihinsel Sabit* (Hamlet'in V. perdedeki eylem-kader felsefesi). Sahnede oyuncu bunu hem replik olarak söyler (metinsel) hem de iç sesinin grameri olarak taşır (zihinsel).

**Macbeth — "Kader yazılmıştır":** Kaynak ekseninde *süreç içi · örtük* (cadılarla karşılaşmadan sonra Macbeth'in iç dünyasında oluşan). Katman ekseninde *Zihinsel Sabit*. Bu inanç olmadan Macbeth'in eylem zinciri kurulamaz.

#### F. Çift Matris Ayrımı (kritik)

ITC içinde iki ayrı 3×3 matris kanondur:

| Konum | Eksenler | Soru |
|---|---|---|
| **§3.10** | VAK × Giriş Kapıları | Oyuncu nasıl öğrenir × Karaktere nereden girer |
| **§3.11** | Kaynak × Katman | Doğru nereden geliyor × Doğru neye dair |

İki matris **karışmamalıdır**. Birincisi oyuncu/karakter ilişkisini, ikincisi doğrunun yapısını ölçer. Farklı modüllerde, farklı pedagojik amaçlarla işler. Karışım önceki dokümanlarda görülmüştür; eğitmen sertifikasyonunda bu ayrımı açıklayabilmek bir kriterdir.

#### G. Çapraz Etki

Bu yapı dört katmanda da uygulanır:

- **Method Book** Bölüm III/Aşama 1 — merkez kavram (Hamlet ve Macbeth matrisleriyle)
- **Workbook v1.0** s.51 — Kaynak Üçlüsü korunur, 2. baskıda Katman Üçlüsüne çapraz referans
- **App** — Hamlet Doğrular sayfası mimarisi yeniden değerlendirilir (§3.10 ve §3.11 nasıl ayrı gösterilecek — Faz 1 küçük revizyon)
- **Workshop** — D2 atölyesi (Karar 9 matrisinden sonra), C2 atölyesi uygular, E1 sertifikasyonu iki matrisi de değerlendirir

### 3.12 · Dijital Araç Sınırı İlkesi (v1.3, Karar 7 — v1.4'te §3.11'den §3.12'ye kaydı)

Yalnızca dijital ortamda anlamlı çalışan araçlar (kişisel test, profil hesaplama, kalibrasyon sonucu üreten testler) App-only olur. Bireyin kâğıt üzerinde kendi başına çalışabileceği araçlar (soru havuzları, defterler, formlar) **kitapta / atölye materyallerinde** yer alır.

**Mevcut Uygulama**

| Araç | Yer | Gerekçe |
|---|---|---|
| Yıldız Oyuncu Testi | App-only | Puan hesaplama + travma erişim eşiği üreten algoritmaya bağlı |
| VAK Testi | App-only | Aynı mantık (kanal puanlaması) |
| MBTI Testi | App-only | Aynı mantık (16 tip hesaplama) |
| Karaktere Hazırlık Soruları | Method Book Bölüm F | Soru havuzu — bireyin kâğıt-kalemle çalıştığı araç |
| Öğrenci Defteri (4 Aşama) | Method Book Bölüm F | Yansıma defteri — bireyin kendi yazdığı araç |

**Workshop Uygulaması**

Atölyede algoritma çalıştırılmaz, manuel egzersiz/defter çalışmaları yapılır; test sonuçları App'ten gelir. **A2 ve sonraki atölyelerin ön koşulu:** katılımcı App Modül I'i tamamlamış olmalı.

---

## 4 · Çapraz Geçişler

Dört projeyi birbirine bağlayan somut köprüler:

### 4.1 · Method Book ↔ Hamlet Workbook

Method Book Workbook'u "yayımlanmış vaka" pozisyonundan referans alır. Her teorik kavramın Workbook karşılığı sayfa numarasıyla verilir:

| Method Book | Workbook karşılığı |
|---|---|
| Açılış (epigraph) | s.7 (ITC İlkesi) · s.18 (etik temel) |
| Bölüm B · Kabul 4 | s.18-19 ("Sahip değil — misafir") |
| Bölüm B · Kabul 6 | s.298-301 (Prova öncesi rutin) |
| Bölüm C · Modül I (Kendini Tanı) | s.31-45 (Kısım I · sezgisel temas) |
| Bölüm C · Modül II / Aşama 2 (Doğrular 3 kaynak) | s.50-57 |
| Bölüm C · Modül II / Aşama 3 (Timeline) | s.58-65 İlişki Haritası · s.66-85 14 sahne timeline |
| Bölüm C · Modül II / Aşama 4 (Çerçeveler) | s.86-105 Yazarın Çerçevesi · s.106-125 Senin Çerçeven |
| Bölüm C · Modül III (Sahneye Götür) tamamı | s.126-289 (Kısım III · 13 yolculuk) |
| Bölüm C · Modül III · Topraklanma adaptasyonu | s.288-289 (Hamlet'e özel 6 adım) |
| Bölüm D · Etik kapanış | s.306-307 |
| Bölüm D · Travma seviyeleri (8×4) | s.20 (2. baskıda 4 yoğunluk olarak güncellenecek) |

### 4.2 · Method Book ↔ ITC App

- **Bölüm C/Modül I (Kendini Tanı)** → App Modül I'in birebir kuramsal karşılığı (VAK + MBTI + Yıldız Oyuncu üçlüsü)
- **Bölüm C/Modül II (Karakteri Tanı)** → App Modül II'nin teorik temeli; karakter sayfası mimarisi paylaşılıyor; Doğrular 3 kaynak yapısı kanon
- **Bölüm C/Modül III (Sahneye Götür)** → App Modül III'ün teorik temeli (Modül III henüz canlıda değil)
- **Bölüm C-Ek (Performans Blokajları)** → App'te potansiyel "Blokaj Aşma" sekmesi (Modül III dışında, ayrı araç)
- **Bölüm D etik altyapı** → App'in arka plandaki güvenlik motoru (8×4 matrisi, Yıldız eşleşme, topraklanma overlay)
- **Bölüm F Karaktere Hazırlık Soruları (9 adım × 3 Kapı)** → App'te potansiyel gelecek katman (Faz 3, henüz karar değil)

### 4.3 · Hamlet Workbook ↔ ITC App

**Modül II Hamlet — REFACTOR CANLI (v1.2)**

App'in Hamlet karakter sayfası, Workbook s.50-125 ile **sayfa-için-sayfa eşleşmiş** durumda:

| App Modül II Hamlet sayfası | Workbook karşılığı |
|---|---|
| Doğrular (3 kaynak yapısıyla yeniden tasarlanacak — Karar 3) | s.50-57 |
| Oyun Öncesi Yaşam · 8 olay + 8 ilişki | s.58-65 |
| Timeline · 14 sahne + Beş Perde temaları | s.66-85 |
| Yazarın Çerçevesi · 5 tercih + sentez | s.86-105 |
| Senin Çerçeven · 5 boşluk × 3 alt-soru hibrit | s.106-125 |

**Modül III Pilot**

Workbook'un 13 yolculuk metni (s.126-289), Modül III'ün ilk pilotu. ~110 dakika tam AI Dış Ses metni.

**Topraklanma**

Workbook s.288-289 Hamlet adaptasyonu, Modül III overlay'inin Hamlet versiyonu olacak.

**VAK + MBTI Profili**

App'te ölçülür, Workbook hazırlık sayfaları VAK kanalına göre okunabilir. 2. baskıda s.31-45 başına MBTI App QR kutusu eklenecek.

**Workbook ↔ App Köprüleri (yeni baskıda eklenecek)**

Workbook bölüm sonlarında "App'te bu bölümün dijital pratiği" kutuları (in-book QR kod). App içinden "Workbook'ta bu sahnenin tam metni" linkleri.

### 4.4 · Üçlü Kesişim (Method Book + Workbook + App)

Aynı oyuncu üç katmanı sırayla deneyimleyebilir:

App ile günlük antrenman → Workbook ile karakter derinleşmesi → Method Kitabı ile yöntemin kuramsal sahibi olma.

Üç katman aynı dili, aynı kavramları, aynı etik altyapıyı paylaşır.

### 4.5 · Method Book ↔ Workshop (YENİ — v1.3)

Workshop, Method Book'un D bölümünden (Eğitmen Rehberliği) türeyen bir uygulama katmanıdır.

| Method Book bölümü | Workshop karşılığı |
|---|---|
| B (Felsefe) | A1 Tanıtım — Kabuller demosu, Manifesto özet |
| C/Modül I (Kendini Tanı) | A2 Modül I bloğu — Kalibrasyon yorumlama (App testleri ön koşul); 6 boyut eğitmen el kitabı |
| C/Modül II (Karakteri Tanı) | A2 Modül II bloğu + B1 (Kendi Karakterin) + C2 (Karakter Sayfası Mimarisi) — 9 adım × 3 Kapı haritası, Doğrular 3 kaynak yapısı |
| C/Modül III (Sahneye Götür) | A2 Modül III bloğu + C3 (Akış & Performans) — Akış katmanı (Csikszentmihalyi) |
| C-Ek (Performans Blokajları) | C3 (Akış & Performans) — ek bölüm Akış'la birlikte öğretilir |
| D (Etik & Eğitmen) | D1 (Etik & Klinik Güvenlik, FKA imzalı) + E1 (Train-the-Trainer) |
| E (Vakalar) | A2'de Macbeth/Hamlet vakalarının uygulamalı işlenişi |
| F (Ekler — Karaktere Hazırlık Soruları) | Atölye katılımcı çalışma kitabı (kâğıt formunda) |

### 4.6 · Workshop ↔ ITC App (YENİ — v1.3)

**A2 ve sonraki atölyeler ön koşul:** Katılımcı App Modül I'i tamamlamış olmalı (VAK + MBTI + Yıldız Oyuncu sonuçları atölye gününe gelmeden hazır).

**Atölyede:** Test sonuçları okunur/yorumlanır, kâğıt üzerinde test çözülmez (Dijital Araç Sınırı — §3.12).

**Atölye sonrası:** Katılımcılar App'ten kalibrasyonlarını derinleştirir, karakter çalışmasını sürdürür.

**B2B/Kurumsal pilot:** Workshop kit + App lisansı paket olarak satılabilir.

### 4.7 · Workshop ↔ Hamlet Workbook (YENİ — v1.3)

- **C2 (Karakter Sayfası Mimarisi) atölyesi:** Workbook'un tam yapısı (Doğrular → Oyun Öncesi Yaşam → Timeline → Çerçeveler) burada öğretilir
- **B1 (Kendi Karakterin) atölyesi:** Katılımcılar kendi karakterleri için Workbook formatını uygular (Hamlet değil, kendi rolleri)
- **A2 amiral gemisi atölyesi:** Hamlet veya Macbeth vakası işlenirken Workbook bir "yayımlanmış vaka" referansı olarak kullanılır

---

## 5 · Açık Karar Noktaları

Aşağıdaki kararlar henüz verilmedi. Bunlardan biri çözüldüğünde Spine güncellenir.

### ✓ v1.3'te çözülen kararlar (artık §3 ve §4'e taşındı)

- MBTI'nın kanonik yeri: ✓ Modül I (§3.9'a eklendi)
- Travma yoğunluğu sayısı: ✓ 4 yoğunluk (§3.2 zaten doğru, dokümante teyit)
- Doğrular yapısı: ✓ 3 kaynak (§3.11'e taşındı; v1.3'te §3.10 idi, v1.4'te kaydı)
- Modül III iç yapısı: ✓ Tek modül (Canlandırma + Akış + Güvenli Çıkış); Performans Blokajları ek bölüm
- Yıldız Oyuncu boyutları: ✓ 6 boyut kanon (§3.9 altında)
- Yıldız Oyuncu test formu yeri: ✓ App-only (§3.12)
- Karaktere Hazırlık Soruları + Öğrenci Defteri yeri: ✓ Method Book Bölüm F (§3.12)
- Karaktere Hazırlık Soruları yapısı: ✓ 9 adım × 3 Giriş Kapısı haritası

### Method Book (açık)

- Alt başlık: "Inside The Character" tek başına mı?
- "Actor's Gym" markasının kitap ile ilişkisi
- Manifesto madde 7-8'in nihai isimleri
- Bölüm I yapısı: Altı Kabul + Manifesto sonda mı, ayrı bölüm mü?
- Çağdaş vaka anonimleştirme derinliği
- Bölüm E klasik vakası: Macbeth mi Hamlet mi (2025 sunum Macbeth pilotluyor)
- FKA imza protokolü: bölüm başı işareti / bölüm sonu küçük not / içindekilerde belirtim

### Hamlet Workbook (açık)

- 2. baskıda Method Book'a geri-referans formatı
- Klinik güvenlik eklemeleri (öz-tarama uzunluğu, "iyi değilsen" dalı formatı)
- Bölüm IX çapa cümlesi ek koruma katmanı
- İngilizce baskı için RSC Shakespeare adaptasyonu

### ITC App (açık)

- AI sesli yönlendirmenin teknik altyapısı (TTS, ElevenLabs voice cloning?)
- Eyüboğlu telifi App'te ticari kullanım için lisans yenilemesi gerekiyor mu? ← Faz 1'de çözülmeli (kritik)
- Method Book ve Workbook ile çapraz pazarlama (in-app linkler, QR)
- Çok-rol katmanı: bireysel/eğitmen/koç/akademi rolleri
- Ticarileşme aşaması: ödeme/abonelik (Stripe + iyzico?), fiyatlandırma şeffaflığı
- Modül II refactor — diğer üç karakter (Macbeth, Willy, Biff) için aynı refactor + Doğrular 3 kaynak yapısı dahil ne zaman? (Faz 3 Milestone 3.2)

### Workshop (açık)

- E1 sertifikasyon yapısı: D1'in ön koşul rolü resmi karar mı, tasarım önerisi mi?
- `Itc_workshop.pdf` eski tasarım: retire / referans / v0 taslak konumu
- 4 hafta ve 12 hafta program şablonları (henüz yazılmamış — 8 haftadan türetilecek)

### Ekosistem genel (açık)

- Method Book Türkçe ilk baskı + İngilizce çeviri sıralaması
- Yayıncı kararı (Routledge, Methuen Drama, Nick Hern Books arasında)
- App'in yurt dışına açılma stratejisi (Stuttgart, RADA bağlantıları)
- Ekosistemin tek bir markası mı (Inside The Character) yoksa dört ayrı isim mi?

---

## 6 · Güncelleme Protokolü

Bu doküman canlı belge. Aşağıdaki olaylar sonrası yeni sürüm çıkar:

- Bir projede yapısal karar değişti (Bölüm prototip onaylandı, Modül canlıya alındı, Workbook 2. baskı çıktı, **yeni katman eklendi** vb.)
- Üç ilke (substitution, travma matrisi, topraklanma) ile ilgili yeni bir karar var
- Çapraz referans tablosunda sayfa/bölüm numaraları değişti
- Yeni bir açık karar noktası ortaya çıktı veya bir tanesi çözüldü
- §3 sabit ilkelere yeni bir kanon ilkesi eklendi

### Sürüm Geçmişi

- **v1.0** — 6 Mayıs 2026 — İlk sürüm. Master Outline v2 ve App canlı durumu temel alındı.
- **v1.1** — 6 Mayıs 2026 — İki büyük karar çözüldü: (1) Modül III pilot kararı — Hamlet Workbook'un 13 yolculuk metni Modül III'ün ilk pilotu olacak. (2) Modül II Hamlet refactor kararı — App'teki mevcut 9 antrenman, Workbook yapısına göre yeniden inşa edilecek. Konumlandırma sentezi açıkça yazıldı: "ticari ama akademik duran". §4.3 (Workbook ↔ App köprüleri) zenginleştirildi.
- **v1.2** — 6 Mayıs 2026 — Modül II Hamlet refactor tamamlandı. Hamlet karakter sayfası Workbook s.50-125 ile birebir hizalandı. §2'de App durumu güncellendi (Modül II Hamlet ✓). §4.3 canlandı (potansiyel köprülerden gerçek bağlantılara). §5'ten "Modül II refactor planı" kaldırıldı, yerine "diğer üç karakterin refactor zamanı" açık karar olarak eklendi.
- **v1.3** — 8 Mayıs 2026 — İki büyük güncelleme: (1) **Workshop dördüncü katman olarak eklendi** — §1, §2, §4 (yeni §4.5-4.7) bu genişlemeye göre güncellendi. (2) **Method Book ana yapı oturumunda 8 stratejik karar alındı** — §3'e üç yeni alt-bölüm eklendi (§3.9 Modül I'in Üç Testi, §3.10 Doğrular Üç Kaynak Yapısı, §3.11 Dijital Araç Sınırı İlkesi). §2 Method Book yapısı güncellendi (C-Ek bölümü eklendi). §5 açık kararları azaldı (9 karar çözüldü).
- **v1.4** — 8 Mayıs 2026 — §3.10 yeniden tanımlandı: Çift Üçlü İlkesi ve 3×3 Matris (VAK × Giriş Kapıları). Eski §3.10 (Doğrular Üç Kaynak Yapısı) §3.11'e, eski §3.11 (Dijital Araç Sınırı İlkesi) §3.12'ye kaydı. §3.9 VAK alt-başlığına ayrım notu eklendi. v1.3'ün geri kalan içeriği aynen korundu.
- **v1.5** — 12 Mayıs 2026 — §3.11 (Doğrular Üç Kaynak) "Doğrular Çift Üçlü ve 3×3 Matris" olarak genişletildi. Karar 3'ün Katman Üçlüsü korundu, Kaynak Üçlüsü eklendi (Workbook s.51 yapısı). İki üçlü çapraz eksenlerde — birlikte 3×3 matris. §3.10'daki VAK × Giriş Kapıları matrisinden farklı bir eksen olduğu vurgulandı. Decision Log Karar 10 kaydı.

---

## 7 · Bu Spine'ın Kullanımı

- Dört projeye Project Knowledge olarak yüklenir (Method Book + Workbook + App + Workshop).
- Yeni bir sohbet açıldığında Claude bu dokümanı okur, ekosistemin tamamını öğrenir.
- Bir karar verildiğinde Beyti tek dokümanı günceller, dört projeye yeniden yükler.
- Bir Claude'un bilmediği şey varsa önce buraya bakar, sonra cevap verir.

**v1.3 itibarıyla:** Decision Log da Spine'ın yanında dört projeye yüklenmeli. Birlikte ekosistemin tam haritasını verirler.

---

*— ITC Ekosistem Spine v1.3 sonu —*
*Inside The Character · Beyti Engin × Filiz Kaya Ataklı · Mayıs 2026*
